// ============================================================
// CONFIGURATION — changer l'URL si tu changes d'hébergeur
// ============================================================
...
// ============================================================
// FETCH INTERCEPTÉ — injecte le mot de passe dans chaque requête
// ✅ Supporte "Bearer <pass>" pour rester compatible avec le bot corrigé
// ============================================================
const _origFetch = window.fetch;
window.fetch = async function(resource, config = {}) {
    config.headers = config.headers || {};
    const pass = localStorage.getItem('dash_pass') || '';
    config.headers['Authorization'] = pass; // bot accepte brut ET Bearer
...
async function tryLogin() {
    ...
    try {
        ...
        alert('❌ Impossible de joindre le serveur. Vérifie que le bot Render est en ligne.');
        localStorage.removeItem('dash_pass');
    }
}
...
function showPage(name, el = null) {
    ...
    if (currentGuild) {
        ...
        if (name === 'invites') { loadInvites(); loadJoinedMembers(); }
        if (name === 'reactionroles') { populateSelects(); loadReactionRoles(); }
        if (['channels','roles','automod','messages','create'].includes(name)) populateSelects();
        if (name === 'welcome' || name === 'rules') loadCurrentConfig();
    }
}

// ============================================================
// INIT — avec gestion cold start Render (retry 3×)
// ============================================================
async function fetchWithRetry(url, opts = {}, retries = 3, delay = 2000) {
    ...
    throw new Error(`Impossible de joindre ${url} après ${retries} tentatives`);
}

async function init() {
    try {
        // ✅ Ping d'abord pour réveiller Render si en cold start
        try { await _origFetch(`${API.replace('/api', '')}/ping`); } catch(_) {}

        const res = await fetchWithRetry(`${API}/guilds`);
        const guilds = await res.json();
        const sel = document.getElementById('guildSelect');
        sel.innerHTML = guilds.map(g =>
            `<option value="${g.id}">${g.name} (${g.member_count} membres)</option>`
        ).join('');
        ...
        toast('Impossible de se connecter à l\'API. Le bot est peut-être en train de démarrer.', 'error');
    }
}
...
async function populateSelects() {
    try {
        ...
        const chOpt = (id) => `<option value="">Aucun</option>` + guildChannels.map(c =>
            `<option value="${c.id}" ${id && currentConfig[id] && String(currentConfig[id]) === String(c.id) ? 'selected' : ''}>#${c.name}</option>`
        ).join('');
        const rOpt = (id) => `<option value="">Aucun</option>` + guildRoles.map(r =>
            `<option value="${r.id}" ${id && currentConfig[id] && String(currentConfig[id]) === String(r.id) ? 'selected' : ''}>${r.name}</option>`
        ).join('');
        const catOpt = `<option value="">Aucune catégorie</option>` + guildCats.map(c =>
...
// ============================================================
// CRÉATION DISCORD (salons, catégories, rôles)
// ============================================================
...
function updatePreview() {
    ...
    document.getElementById('previewDesc').textContent = (document.getElementById('welcomeMessage').value || 'Salut {user} ! Tu es notre {count}ème membre ! 🎉')
        .replace('{user}', '@NouvelUtilisateur').replace('{username}', 'NouvelUtilisateur')
        .replace('{count}', '42').replace('{server}', 'Mon Serveur');
...
async function saveWelcomeConfig() {
    ...
    if (title) patch.welcome_title = title;
    if (color) patch.welcome_color = color;
...
async function testWelcome() {
    try { await fetch(`${API}/test/welcome/${currentGuild}`, { method: 'POST' }); toast('Message envoyé !'); }
    catch(e) { toast('Erreur.', 'error'); }
}
...
async function saveRules() {
    ...
    if (title) patch.rules_title = title;
    if (text) patch.rules_text = text;
    if (Object.keys(patch).length > 0) {
        await saveConfig(patch);
        toast('Règles sauvegardées ! Refais /setup_rules dans Discord.');
    } else {
        toast('Aucune modification à sauvegarder', 'error');
    }
}
...
function renderLevelRoles() {
    ...
    c.innerHTML = `<table><thead><tr><th>Niveau</th><th>Rôle</th><th>Action</th></tr></thead><tbody>` +
        Object.entries(lr).sort((a,b)=>a[0]-b[0]).map(([lv, rId]) => {
            const role = guildRoles.find(r => r.id == rId);
            return `<tr><td><span class="badge badge-blue">Niv ${lv}</span></td><td>${role ? role.name : rId}</td><td><button class="btn btn-danger" onclick="removeLevelRole('${lv}')">✕</button></td></tr>`;
        }).join('') + `</tbody></table>`;
}

async function addLevelRole() {
    const lv = document.getElementById('levelNum').value;
    const rId = parseInt(document.getElementById('levelRole').value);
    if (!lv || !rId) return toast('Remplis tout', 'error');
    if (!currentConfig.level_roles) currentConfig.level_roles = {};
    currentConfig.level_roles[lv] = rId;
    await saveConfig({ level_roles: currentConfig.level_roles });
    renderLevelRoles();
    toast('Ajouté !');
}

async function removeLevelRole(lv) {
    delete currentConfig.level_roles[lv];
    await saveConfig({ level_roles: currentConfig.level_roles });
    renderLevelRoles();
}
...
async function saveAutomod() {
    await saveConfig({ anti_spam: document.getElementById('antiSpamToggle').checked });
    toast('Automod maj !');
}

function renderBannedWords(w) {
    document.getElementById('bannedWordsList').innerHTML = w.map(x =>
        `<span class="badge badge-red" style="padding:6px; font-size:12px">${x} <span style="cursor:pointer" onclick="removeBannedWord('${x}')">✕</span></span>`
    ).join('');
}
...
async function removeBannedWord(w) {
    ...
    await saveConfig({ banned_words: currentConfig.banned_words });
    renderBannedWords(currentConfig.banned_words);
}
...
async function createRRMessage() {
    ...
    try {
        await fetch(`${API}/reaction_roles_create`, {
            method: 'POST', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ channel_id, guild_id: currentGuild, title, description, pairs: valid })
        });
        rrPairs = []; renderRRPairs(); await loadCurrentConfig(); loadReactionRoles();
        toast('Message RR créé !');
    } catch(e) { toast('Erreur', 'error'); }
}
...
async function loadReactionRoles() {
    ...
    c.innerHTML = Object.entries(rr).map(([k, rId]) => {
        const parts = k.split('_'); const emoji = parts.slice(1).join('_');
        const role = guildRoles.find(r => r.id == rId);
...
async function removeRR(k) {
    delete currentConfig.reaction_roles[k];
    await saveConfig({ reaction_roles: currentConfig.reaction_roles });
    loadReactionRoles();
}
...
async function sendEmbedMessage() {
    ...
    await fetch(`${API}/send_message`, {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ channel_id, embed:{ title, description, color } })
    });
    toast('Embed envoyé !');
}
...
async function sendPoll() {
    ...
    const options = ['poll_opt1','poll_opt2','poll_opt3','poll_opt4']
        .map(id => document.getElementById(id).value).filter(Boolean);
    if (!channel_id || !question || options.length < 2)
        return toast('Il faut un salon, une question et 2 options minimum', 'error');
    await fetch(`${API}/create_poll`, {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ channel_id, question, options })
    });
    toast('Sondage créé !');
}
...
async function loadInvites() {
    ...
    document.getElementById('invitesTable').innerHTML = !sorted.length
        ? `<p>Aucune donnée</p>`
        : `<table><thead><tr><th>User ID</th><th>Invitations</th></tr></thead><tbody>`
          + sorted.map(([u,d]) => `<tr><td>${u}</td><td><span class="badge badge-green">${d.count} invites</span></td></tr>`).join('')
          + `</tbody></table>`;
}
...
async function loadJoinedMembers() {
    try {
        ...
        for (const [memberId, info] of Object.entries(data)) {
            ...
            row.innerHTML = `<td>${memberId}</td><td>${inviterId}</td><td>${isValid ? '✅ Oui' : '⚠️ Faux compte'}</td>`;
...
async function loadWarns() {
    ...
    document.getElementById('warnsTable').innerHTML = !sorted.length
        ...
        : `<table><thead><tr><th>User ID</th><th>Warns</th><th>Dernière raison</th><th>Action</th></tr></thead><tbody>`
          + sorted.map(([u,w]) => `<tr><td>${u}</td><td><span class="badge badge-red">${w.length}</span></td><td>${w[w.length-1].raison}</td><td><button class="btn btn-danger" style="padding:4px 10px;font-size:11px" onclick="popWarn('${u}')">−1 Warn</button></td></tr>`).join('')
          + `</tbody></table>`;
}

async function popWarn(uid) {
    await fetch(`${API}/warns/${currentGuild}/${uid}/pop`, { method: 'POST' });
    loadWarns();
    toast('Warn retiré !');
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ['welcomeTitle','welcomeMessage','welcomeColor','welcomeShowInviter'].forEach(id => {
    ...
    if (savedPass) {
        try {
            ...
            // Render est peut-être en cold start — on laisse l'overlay visible
        }
    }
});